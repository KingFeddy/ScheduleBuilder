...
test('restores preferences before saving and uses current settings for both generate actions', async ({ page, api }) => {
  ...
  expect(await page.evaluate(() => JSON.parse(localStorage.getItem('njit-dw-preferences') || '{}')))
    .toEqual({ version: 1, courses: ['CS435'], creditsPerSemester: 12, startTerm: null })
  ...
  expect(api.requests('POST', '/api/plan/generate')[0].postDataJSON().preferences).toEqual({ courses: ['CS435'], credits_per_semester: 12, start_term: '202690' })
  await page.reload()
  await expect(credits).toHaveValue('12')
  await credits.fill('9')
  await page.getByRole('button', { name: 'Regenerate', exact: true }).click()
  await expect.poll(() => api.requests('POST', '/api/plan/generate').length).toBe(2)
  expect(api.requests('POST', '/api/plan/generate')[1].postDataJSON().preferences).toEqual({ courses: ['CS435'], credits_per_semester: 9, start_term: '202690' })
  ...
  expect(api.requests('POST', '/api/plan/generate')[2].postDataJSON().preferences).toEqual({ courses: [], credits_per_semester: 9, start_term: '202690' })
})
...
test('uses in-memory preferences when browser storage is unavailable', async ({ page, api }) => {
  ...
  await page.getByRole('button', { name: 'Generate My Plan', exact: true }).click()
  await expect(page.getByRole('heading', { name: 'Your Academic Plan' })).toBeVisible()
  expect(api.requests('POST', '/api/plan/generate')[0].postDataJSON().preferences).toEqual({ courses: ['CS435'], credits_per_semester: 9, start_term: '202690' })
